    ...
    <div id="content_box">
      <div class="search_line">
        <div class="shuru_top">
          ...
          <div class="shurubox" v-if="this.userid === 'System'">
              <span  class="twice_search">车间：</span>从
                  <el-select style="width:130px;margin:0 10px;" clearable @change="chooseLab" v-model="requestData.value"  placeholder="请选择" size="small" class="minibox">
                  ...
                  <el-select style="width:130px;margin:0 10px;" clearable @change="chooseLab1" v-model="requestData.value2"  placeholder="请选择" size="small" class="minibox">
          ...
          <div class="shurubox" v-else><!--除了System之外的其他用户，车间直接固定为当前用户的-->
              <span  class="twice_search">车间：</span>从<!--固定车间查询值之后。改变v-model的值，加disabled，不允许更改-->
                  <el-select disabled style="width:130px;margin:0 10px;" clearable @change="chooseLab" v-model="fixedClass"  placeholder="请选择" size="small" class="minibox">
                    <el-option  v-for="(item,index) in options1" :key="index" :label="item.wk_workshop_desc" :value="item.wk_workshop_id" class="minbox1"></el-option>
                  </el-select>至
                  <el-select disabled style="width:130px;margin:0 10px;" clearable @change="chooseLab1" v-model="fixedClass"  placeholder="请选择" size="small" class="minibox">
                    <el-option  v-for="(item,index) in options2" :key="index" :label="item.wk_workshop_desc" :value="item.wk_workshop_id" class="minbox1"></el-option>
                  </el-select>
              <span  class="twice_search">班别：</span><el-select style="width:130px;margin:0 23px;" clearable v-model="requestData.value1" placeholder="请选择" size="small" class="minibox" @change="chooseVal">
                <el-option v-for="(item,index) in options" :key="index" :label="item.wk_classes_desc" :value="item.wk_classes_id" class="minbox1"></el-option>
      ...
      <div class="table_box"  >
        <!--  === 'wk__chr01'?'人工工时合计' : (item.prop === 'wk__chr02' ? '损失工时合计' : item.prop )  -->
          <el-table id="table_data" stripe @row-click="dianji" highlight-current-row :max-height="maxHeight"  :data="tableData" border  >
              <el-table-column v-for="(item,index) in tableHeader" :min-width="item.prop.length > 8 ? '100px' : item.prop === '日期' || item.prop === '创建时间' || item.prop === '修改时间' ? '170px' : '0'" align="center" :key="index" :label="item.prop"
                >
                 <!-- <template v-if="item.prop !== 'wk_mstr_id' && item.prop !=='wk__chr01' && item.prop !== 'wk__chr02'"> -->
                  <template slot-scope="scoped">
                    <!-- 要隐藏两个字段 -->
                  <!-- <p class="table_tag" >{{scoped.row[item.prop] == 'wk__chr01' ?'损失人工工时':(scoped.row[item.prop] == 'wk__chr02'? '损失工时': scoped.row[item.prop] ) }}</p> -->
                  <p class="table_tag" v-if="item.prop !== 'wk_mstr_id'">{{scoped.row[item.prop]}}</p>
                  </template>
                <!-- </template> -->
              </el-table-column>
          </el-table>
...
<script>
import method from './utils/getTime'
export default {
  ...
  data(){
   ...
   created(){
        var userid = JSON.parse(localStorage.getItem("target")).userId; //用户id
        this.userid=userid;
        this.nowTime = String(new Date().getFullYear())+String(new Date().getMonth()+1)+String(new Date().getDate());//现在的时间
        //  nowTime:'',
        // createTime:'',
        this.setSize1();
        //在mouted后调用一下查询
        this.searchData();
        var a=method.getTime();
        console.log("更改")
  },
  mounted(){
    this.fixedClass = JSON.parse(localStorage.getItem("target")).telephone;//拿到用户的车间
    console.log("当前用户的车间")
    console.log(this.fixedClass)
    var date=this.getTime2();
    this.requestData.time=date;
    this.requestData.time1=date;
    let that = this;
  ...
  methods:{
    ...
    edit(){
        if(this.msgId.wk_id){
          var timeStr=this.msgId['日期'].toString();
          var createTime = new Date(timeStr);
          var createTimeStr= String(createTime.getFullYear())+String(createTime.getMonth()+1)+String(createTime.getDate());
          if(this.userid === 'System' || (this.userid === this.msgId.wk_userid && createTimeStr === this.nowTime)){
            this.$router.push({path:'/table',query:{id:this.msgId}});
          }else{
            this.$message("您没有权限编辑")
          }
        }else(this.$message("请选中数据"))   
    },
    getTime(){
        var time = new Date();
        var year = time.getFullYear();
        var month = time.getMonth() + 1 >= 10 ? time.getMonth() : "0" + time.getMonth();
        var day = time.getDate() >= 10 ? time.getDate() : "0" + time.getDate();
        ...
        var exTime = `${year}-${month}-${day} ${hour}:${minute}:${second}`
        return exTime;
    },
    tableDelet(){
      // var userid
      ...
      var createTime = new Date(timeStr);
      var createTimeStr= String(createTime.getFullYear())+String(createTime.getMonth()+1)+String(createTime.getDate());
      console.log("现在的时间："+this.nowTime);
      console.log("表单的创建时间："+createTimeStr);
      console.log("创建人："+this.msgId.wk_userid);
        if(this.msgId.wk_id){
        console.log(this.msgId);
        console.log(this.msgId['日期'])
        var timeStr=this.msgId['日期'].toString();
          if(this.userid === 'System' || (this.userid === this.msgId.wk_userid && createTimeStr === this.nowTime)){
            // let baseUrl = 'http://39.104.64.133:8080'
            let projUrl = '/Home/Index';
            ...
            var date =this.getTime(); //操作时间
            $.ajax({
              url: baseUrl +'/Qzd_CodeDemo/wk_mstr/DeleteForm',
              type: 'POST', // GET
              async: true,
              data:{ keyValue:this.msgId.wk_id, date: date, userid: userid },
              dataType: 'json',
              success: function(res) {
                  that.searchData()
                  that.$message("删除成功")
    ...
    searchData(){
      ...
      let wk_workshop_id = this.userid === 'System' ? JSON.stringify(this.requestData.newLable) :JSON.stringify(JSON.parse(localStorage.getItem("target")).telephone) //从车间
      let wk_workshop_id1 = this.userid === 'System' ? JSON.stringify(this.requestData.newLable1) :JSON.stringify(JSON.parse(localStorage.getItem("target")).telephone)//至车间
      console.log("此时的参数")
      console.log(wk_workshop_id)
      console.log(wk_workshop_id1)
      ...
      $.ajax({
        ...
        success: function(res) {
          if(res.code === 200 && res.data.rows.length > 0){
            ...
            for(let tem  in row ){
              //忽略的列
             if(tem !== 'wk_id' && tem !== 'wk_workshop_id' && tem !== 'wk_classes_id'&& tem !== 'wkd_wk_id' && tem.trim() !== 'recordtotal' && tem !== '行号' && tem.trim() !== 'wk_mode_id'  && tem !== 'wk_unhealthy_id' && tem !== 'wk_userid' 
              && tem !== 'wk_mstr_id' && tem !== 'wk__chr01' && tem !== 'wk__chr02' 
             ){
